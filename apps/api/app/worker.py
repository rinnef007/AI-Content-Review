...
def handle(payload: dict) -> dict:
    ...
    if kind == "review_script":
        ...
        try:
            ...
            analysis = dict(episode.analysis or {})
            analysis["review_script"] = script
            episode.analysis = analysis
            db.commit()
            return {"episode_id": episode.id, "review_script": script}
        finally:
            db.close()
    ...
    if kind == "media_episode_analysis":
        ...
        media_type = media_kind(payload.get("filename", path.name)); metadata = probe_media(str(path)); audio_path = path.with_suffix(".wav")
        extract_audio(str(path), str(audio_path)); transcript = transcribe_audio(str(audio_path)); content = transcript.get("text", "").strip()
        ...
        request = AnalyzeRequest(title=payload["title"], content=content, mode="review", spoiler=False); result = analyze_content(request)
        result["media"] = {"type": media_type, "filename": payload.get("filename"), "metadata": metadata}; result["transcript"] = transcript
        db = SessionLocal()
        try:
            episode = db.get(Episode, payload["episode_id"])
            if not episode or episode.project_id != payload["project_id"]: raise ValueError("Episode not found")
            episode.content = content; episode.source_filename = payload.get("filename"); episode.source_type = media_type; episode.analysis = result
            persisted = persist_analysis(db, payload["project_id"], episode, result); db.commit(); return {"episode_id": episode.id, "analysis": result, "persisted": persisted}
        finally: db.close()
