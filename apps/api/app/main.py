...
async def lifespan(_: FastAPI):
    init_db(); UPLOAD_DIR.mkdir(parents=True, exist_ok=True); yield

app = FastAPI(title="AI Content Review API", version="1.1.0", lifespan=lifespan)
...
def local_analyze(payload: AnalyzeRequest) -> dict:
    words = payload.content.split(); sentences = [s.strip() for s in re.split(r"[.!?]+", payload.content) if s.strip()]
...
def extract_pdf(data: bytes) -> tuple[str, int, bool]:
    ...
    text_parts = [(page.extract_text() or "").strip() for page in reader.pages]; text = "\n\n".join(p for p in text_parts if p)
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "service": "api", "version": "1.1.0", "ocr": "tesseract", "ai_provider": AI_PROVIDER, "queue": "redis", "media": "ffmpeg", "srt": True}

@app.get("/api/v1/projects")
def list_projects(db: Session = Depends(get_db)) -> list[dict]: return [project_dict(p) for p in db.scalars(select(Project).order_by(Project.created_at.desc())).all()]
...
def create_episode(project_id: str, payload: EpisodeCreate, db: Session = Depends(get_db)) -> dict:
    ...
    episode = Episode(project_id=project_id, title=payload.title, content=payload.content, source_type=payload.source_type); db.add(episode); db.commit(); db.refresh(episode)
...
def upload_episode(project_id: str, file: UploadFile = File(...), title: str | None = Form(default=None), db: Session = Depends(get_db)) -> dict:
    ...
    path, filename, _ = save_upload(file, MAX_UPLOAD_BYTES); episode = Episode(project_id=project_id, title=title or Path(filename).stem, source_type="file", source_filename=filename); db.add(episode); db.commit(); db.refresh(episode)
    return {"episode": {"id": episode.id, "project_id": project_id, "title": episode.title, "source_type": episode.source_type, "source_filename": filename}, "job": enqueue({"kind": "upload_episode_analysis", "project_id": project_id, "episode_id": episode.id, "file_path": path, "filename": filename, "title": episode.title, "mode": "review", "spoiler": False})}
...
def upload_media_episode(project_id: str, file: UploadFile = File(...), title: str | None = Form(default=None), db: Session = Depends(get_db)) -> dict:
    ...
    path, filename, size = save_upload(file, MAX_MEDIA_BYTES); media_type = "video" if suffix in {".mp4", ".mov", ".mkv", ".webm", ".avi", ".m4v"} else "audio"
    episode = Episode(project_id=project_id, title=title or Path(filename).stem, source_type=media_type, source_filename=filename); db.add(episode); db.commit(); db.refresh(episode)
    return {"episode": {"id": episode.id, "project_id": project_id, "title": episode.title, "source_type": media_type, "source_filename": filename, "size": size}, "job": enqueue({"kind": "media_episode_analysis", "project_id": project_id, "episode_id": episode.id, "file_path": path, "filename": filename, "title": episode.title})}
...
def create_analysis_job(payload: AnalyzeRequest) -> dict: return enqueue({"kind": "analysis", **payload.model_dump()})
...
def get_transcript(project_id: str, episode_id: str, db: Session = Depends(get_db)) -> dict:
    episode = db.get(Episode, episode_id)
    if not episode or episode.project_id != project_id: raise HTTPException(status_code=404, detail="Episode not found")
...
def get_srt(project_id: str, episode_id: str, db: Session = Depends(get_db)) -> str:
    episode = db.get(Episode, episode_id)
    if not episode or episode.project_id != project_id: raise HTTPException(status_code=404, detail="Episode not found")
...
def list_relationships(project_id: str, db: Session = Depends(get_db)) -> list[dict]:
    if not db.get(Project, project_id): raise HTTPException(status_code=404, detail="Project not found")
    relationships = db.scalars(select(Relationship).where(Relationship.project_id == project_id)).all(); characters = {c.id: c.name for c in db.scalars(select(CharacterModel).where(CharacterModel.project_id == project_id)).all()}
...
def analyze(payload: AnalyzeRequest) -> dict: return enqueue({"kind": "analysis", **payload.model_dump()})
