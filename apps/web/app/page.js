...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        const current = await request(`/api/v1/jobs/${job.id}`);
        setJob(current);
        if (current.status === "completed") {
          await loadProject();
          setMessage("Job hoàn tất: OCR và phân tích AI đã xử lý xong.");
          setTab("characters");
        }
        if (current.status === "failed") setError(current.error || "Job xử lý thất bại");
      } catch (err) { setError(err.message); }
    }, 1500);
  ...
  async function uploadEpisode() {
    ...
    try {
      ...
      if (episodeTitle.trim()) form.append("title", episodeTitle.trim());
      const data = await request(`/api/v1/projects/${projectId}/episodes/upload`, { method: "POST", body: form });
      setFile(null); setEpisodeTitle(""); await loadProject(); setJob(data.job); setMessage("Đã nhận file. OCR và AI đang chạy nền."); setTab("episodes");
    } catch (err) { setError(err.message); } finally { setLoading(false); }
  }
  ...
  async function analyzeEpisode(id) {
    ...
    try {
      ...
      setJob(data.job); setMessage("Đã đưa episode vào hàng đợi AI.");
    } catch (err) { setError(err.message); } finally { setLoading(false); }
  }
  ...
  return (
    <main style={styles.page}>
      <header style={styles.header}>
        <div><p style={styles.eyebrow}>AI CONTENT REVIEW</p><h1 style={styles.h1}>Project Knowledge</h1><p style={styles.subtitle}>OCR → Queue → AI → Knowledge Base</p></div>
      ...
      {job && <section style={styles.job}><strong>Job: {job.status}</strong><span>{job.attempts || 0}/{job.max_attempts || 3} attempts</span>{job.error && <small>{job.error}</small>}</section>}
      ...
      <footer style={styles.footer}>API: {API} · Redis Queue · OCR · Knowledge Base · PostgreSQL</footer>
    </main>
  );
}
...
const styles = {
  ...
  job: { display: "flex", gap: 18, alignItems: "center", padding: 12, border: "1px solid #ddd", borderRadius: 10, marginBottom: 14, background: "#fafafa" },
