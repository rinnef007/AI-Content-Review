...
export default function Home() {
  ...
  async function loadProject(id = projectId) {
    if (!id) return;
    try { setProject(await request(`/api/v1/projects/${id}`)); } catch (err) { setError(err.message); }
  }
  ...
  async function analyzeEpisode(id) {
    ...
    try {
      ...
      await loadProject(); setTab("characters"); setMessage(`Đã phân tích: ${data.persisted.characters_created || 0} nhân vật mới, ${data.persisted.events_created || 0} sự kiện.`);
    } catch (err) { setError(err.message); } finally { setLoading(false); }
  }
  ...
  return (
    <main style={styles.page}>
      ...
      <section style={styles.topGrid}>
        ...
        <div style={styles.card}><strong>{project?.name || "Chưa chọn project"}</strong><p style={styles.muted}>{project?.description || "Tạo project để bắt đầu quản lý series."}</p><div style={styles.stats}><span>{episodes.length} Episodes</span><span>{characters.length} Characters</span><span>{events.length} Events</span></div></div>
      </section>

      <section style={styles.tabs}>{[["overview","Tổng quan"],["episodes","Episodes"],["characters","Nhân vật"],["events","Sự kiện"]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} style={tab === key ? styles.activeTab : styles.tab}>{label}</button>)}</section>
...
const styles = {
  ...
  tabs: { display: "flex", gap: 8, margin: "18px 0", borderBottom: "1px solid #ddd", paddingBottom: 8 },
  ...
  stats: { display: "flex", gap: 18, color: "#555", fontSize: 14 },
